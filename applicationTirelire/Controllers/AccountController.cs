...
namespace applicationTirelire.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public void AssocierClient(string mail)
        {
            try
            {
                IRepository<User> client = new EFRepository<User>();
                User cl = client.Lister().Where(c => c.emailUser == mail).First();
                Session["User"] = cl;
            }
            catch (Exception)
            {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            // Si nous sommes arrivés là, un échec ²’est produit. Réafficher le formulaire
            return View(model);
        }

        private void CreerClient(RegisterViewModel model)
        {
            IRepository<User> repClient = new EFRepository<User>();
            try
            {
                User nouveau = new User()
                {
                    ...
                    sexe = model.sexe,
                    adresseUser = model.adresseUser,
                    villeUser = model.villeUser,
                    codePostal = model.codePostal,
                    paysUser = model.paysUser,
                    telephoneUser = model.telephoneUser,
                    emailUser = model.Email,
                    statusUser = 1
                };
                Session["User"] = repClient.Ajouter(nouveau);
            }
